...
DATA_DIR = Path(__file__).parent.parent / "data" / "processed"  # Fixed: Point to data/processed/
...
def load_data():
    ...
    feature_cols = [col for col in train_df.columns if col not in [label_col, 'timestamp', 'symbol']]
...
def main():
    ...
    results['logistic'], lr_model = train_logistic_regression(X_train, X_test, y_train, y_test)
    
    # 3. Random Forest (Production)
    results['random_forest'], rf_model = train_random_forest(X_train, X_test, y_train, y_test, feature_cols)
    ...
    for model_name, metrics in results.items():
        print(f"{model_name:<25} {metrics['f1_score']:<8.4f} {metrics.get('pr_auc', 0):<8.4f} "
    ...
    print(f"✅ View experiments in the 'mlruns' directory.")
    print("\n" + "=" * 70)
