...
def main():
    ...
    default_test_data_path = Path(__file__).parent.parent / "data" / "processed" / "test.parquet"  # Fixed: Point to data/processed/

    parser = argparse.ArgumentParser(description='Crypto Volatility Inference')
    parser.add_argument('--model', type=str, help='Path to model file', default=str(default_model_path))
    ...
    parser.add_argument('--window', type=int, default=60,
                       help='Window size in seconds for benchmark')
    ...
    # Prepare features
    feature_cols = [col for col in test_df.columns if col not in ['volatility_spike', 'timestamp', 'symbol']]
    X_test = test_df[feature_cols]
    
    if 'volatility_spike' in test_df.columns:
        y_test = test_df['volatility_spike']
        print(f"   Samples: {len(X_test):,} | Positive: {y_test.sum():,} ({y_test.mean()*100:.2f}%)")
    else:
    ...
    print(f"   📊 Detected {spike_count:,} volatility spikes ({spike_count/len(predictions)*100:.2f}%)")
    
    # Show sample predictions
    print(f"\n   📋 Sample predictions (first 5):")
    for i in range(min(5, len(predictions))):
        label = f"(Actual: {y_test.iloc[i]})" if 'volatility_spike' in test_df.columns else ""
        print(f"      Sample {i+1}: Pred={predictions[i]}, Prob={probabilities[i]:.4f} {label}")
    
    # Evaluate if labels are available
    if 'volatility_spike' in test_df.columns:
        eval_metrics = predictor.evaluate_predictions(y_test, predictions, probabilities)
    
    # Benchmark if requested
    if args.benchmark:
        metrics = predictor.benchmark_inference_speed(X_test, window_seconds=args.window)
